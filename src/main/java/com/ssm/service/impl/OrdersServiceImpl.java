...
public class OrdersServiceImpl implements OrdersServiceInter {
	...
	public String addOrders(String lists) throws Exception {
		Map<String, String> map = new HashMap<String, String>();
		 ...
		 String jsonSubStr = lists.substring(6, lists.length());//分割截取把头部截掉
		 int sum = 1;
		 JSONArray array = JSONArray.parseArray(jsonSubStr);//截取掉后转成list数组
		 for(int i=0;i<array.size();i++) {
			 JSONObject userJson = JSONObject.parseObject(array.getString(i));//遍历没一个
			 GoodsOrders goodsOrders = JSON.toJavaObject(userJson,GoodsOrders.class);//将每一个json转为对应实体类
			 goodsOrders.setOrder_date(Time.getTimes());//生成时间戳
			 goodsOrders.setOrder_code(Time.getTimeCode());//生成订单号
