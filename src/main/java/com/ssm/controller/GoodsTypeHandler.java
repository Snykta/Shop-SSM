...
public class GoodsTypeHandler {
	...
	public void addType(GoodsType type,HttpServletResponse response) throws Exception{
		...
		if(flag) {
			...
			response.getWriter().println("<script>alert('Ìí¼ÓÊ§°Ü£¡');location.href='/Shop-SSM/admin/add_type.html';</script>");
		}
	...
	public void AdminUpdateType(GoodsType type,HttpServletResponse response) throws Exception{
		...
		if(flag) {
			...
			response.setContentType("text/html;charset=utf-8");
			response.getWriter().println("<script>alert('ÐÞ¸ÄÊ§°Ü£¡');location.href='/Shop-SSM/admin/goods_sort.html';</script>");
		}
